...
class StoreLote extends Component {
    ...
    render(){
        ...
        if(this.state.lote.meta.total > 10){
            for(var i=1; i <= this.state.lote.meta.last_page; i++){
                pages.push(
                <li key={i} className={this.state.lote.meta.current_page === i?"page-item disabled":"page-item"} >
                    <a href="#e" id={i} className="page-link" onClick={this.handleChangePage.bind(this)} >{i}</a>
                </li>
                );
            }
        }
                    ...
                    <table className="table table-hover table-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th className="text-right" style={{width:90}} >Unidades</th>
                                <th className="text-right" style={{width:110}} >Costo</th>
                                ...
                                ? lote.data.map(lot => {
                                    return(
                                        <tr key={lot.id}>
                                            ...
                                            <td className="text-uppercase">{lot.factura?lot.factura:'XXXX'}</td>
                                            <td className="text-right">{lot.cantidades}</td>
                                            <td className="text-right">$ {lot.costo?lot.costo:0}</td>
                                            <td className="text-right">$ {lot.precio?lot.precio:0}</td>
                                            <td>
                                                <a className="btn-flat text-warning" 
                                                    href="#delete" 
                                                    onClick={this.handleDelete.bind(this)}
                                                    id={lot.id} >
                                                    <i className="fas fa-trash" id={lot.id} ></i>
                                                </a>
                                            </td>
                                        </tr>
                                    );
    ...
    handelGetLots(page){
        ...
        let varLocalStorage = JSON.parse(localStorage.getItem('OrusSystem')),
            id = this.props.id,
            url = "http://"+ varLocalStorage.host +"/api/items?store_items_id="+id;
        fetch(url+"&page="+page,{
            method: 'GET',
            headers: {
                'Accept': 'application/json',
