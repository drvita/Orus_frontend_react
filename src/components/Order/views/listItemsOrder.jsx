...
export default function ListItemComponent(props) {
  ...
  const validateList = () => {
    if (productCodes) {
      let valid = productCodes.some(
        (code) => parseInt(code) === 1 || parseInt(code) === 2
      );
      setState({
        ...state,
        validList: valid,
  ...
  const handleAddItem = (item) => {
    ...
    const { items: oldItems, ChangeInput: _changeInput } = props;
    ...
    if (itemFound !== -1) {
      const subtotal = items[itemFound].price * cant;
          ...
          {!showHideBtns ? (
            <div className="d-flex justify-content-end mb-2">
              <button
                ...
                disabled={state.validList === false ? true : false}
              >
                Cancelar
              </button>
              <button
